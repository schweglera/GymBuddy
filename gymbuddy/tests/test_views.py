...
class ArticleListViewTests(TestCase):
    ...
    def test_homepage_view_renders_correctly(self):
        response = self.client.get(reverse("homepage"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Willkommen bei Gym-Buddy!")
        self.assertContains(response, self.article.title)

    def test_homepage_view_no_articles(self):
    ...
    def test_homepage_view_without_admin_button_for_normal_user(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("homepage"))
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, "Coach hinzufügen")

    def test_homepage_view_with_admin_button_for_admin_user(self):
